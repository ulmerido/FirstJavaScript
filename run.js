...
export class UIMannager
{
    ...
    _linkBoardToHTML()
    {
        let f1 = this.onMouseLeave_Cell;
        f1.bind(this);
        let f2 = this.onMouseEnter_Cell;
        f2.bind(this);
        let f3 = this.onMouseClick_Cell;
        f3.bind(this);
        for (let i = 0; i < this.m_Size; i++)
        {
            for (let j = 0; j < this.m_Size; j++)
            {
                ...
                this.m_Game.Board[i][j].Cell.addEventListener('mouseleave', function () { f1(); });
                this.m_Game.Board[i][j].Cell.addEventListener('mouseenter', function () { f2(i, j); });
                this.m_Game.Board[i][j].Cell.addEventListener('click', function () { f3(i, j); });
            }
        }
    }
